r"""Ed25519 verification — a vendored copy of packaging/ed25519.py.

Everything below this docstring is that file. It is copied rather than imported
because MisteryUpdate.exe is frozen on its own: packaging\ is a build-time
folder that never ships, and an updater that had to import from it would be an
updater that could not run on anyone's PC.

Copying is only safe if the copy cannot drift, so packaging\test_updater.py
compares the two files as syntax trees — identical code, comments and wording
allowed to differ — signs with one and verifies with the other over random
keys, and checks both against `cryptography`, which is OpenSSL. A signer and a
verifier that disagree is the kind of thing you find out about on release day,
from someone else.

Only verify() runs in the updater. sign(), public_key() and
generate_private_key() come along because they are part of the file being
copied, and because the tests use them.
"""
